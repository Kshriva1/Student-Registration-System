...
--Sequence->sequence.sql: Varun Saxena
--Query1
...
--Package:Varun Saxena
--Query4
procedure get_prerequisites (v_dept_code in courses.dept_code%type,
			     v_course# in courses.course#%type,
		  	     error_message out varchar2,
			     r_cursor out sys_refcursor);


--Package:Kundan Shrivastav
--Query5
...
--Stored Procedure: show_students
procedure show_students (student_cursor out sys_refcursor) AS
BEGIN
        open student_cursor for
        select * from students;
END;

--Stored Procedure: show_courses
procedure show_courses (student_cursor out sys_refcursor) AS
BEGIN
        open student_cursor for
        select * from students;
END;

--Stored Procedure: show_TAs
procedure show_TAs (student_cursor out sys_refcursor) AS
BEGIN
        open student_cursor for
        select * from TAs;
end;

--Stored Procedure: show_classes
procedure show_classes (student_cursor out sys_refcursor) AS
BEGIN
        open student_cursor for
        select * from classes;
END;

--Stored Procesure: show_enrollments
procedure show_enrollments (student_cursor out sys_refcursor) AS
BEGIN
        open student_cursor for
        select * from enrollments;
END;

--Stored Procedure: show_prerequistes
procedure show_prerequisites (student_cursor out sys_refcursor) AS
BEGIN
        open student_cursor for
        select * from prerequisites;
END;

--Store Procedure: show_logs
procedure show_logs(student_cursor out sys_refcursor) AS
BEGIN
        open student_cursor for
        select * from logs;
END;
...
--Store Procedure: ta_info
procedure ta_info (v_classid in Classes.classid%type, error_message out varchar2, r_cursor out sys_refcursor) AS	
...
--Store Procedure: get_prerequisites
procedure get_prerequisites (v_dept_code in courses.dept_code%type,
                             v_course# in courses.course#%type,
                             error_message out varchar2,
...
procedure enroll_student (v_B# in students.B#%type,v_classid in classes.classid%type,error_message out varchar2) AS
v_student_B# Number;
...
--Stored Procedure: drop_student
procedure drop_student (dropB# in Students.B#%type,dropClassid in Classes.Classid%type,error_message out varchar2) AS
